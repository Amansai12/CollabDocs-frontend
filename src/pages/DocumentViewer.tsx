...
function DocumentViewer() {
    ...
    const [rollbackConfirmDialogOpen, setRollbackConfirmDialogOpen] = useState(false);
    ...
    ws.onmessage = (e: MessageEvent) => {
      ...
      switch(data.type) {
        ...
        case 'version':
            ...
            break
        case 'delete-version':
            setVersions((prev) => prev.filter((version) => version.id !== data.versionId))
            if(myRole != 'VIEWER'){
                toast({
  ...
  useEffect(() => {
    ...
    const shareWithUser = async () => {
        ...
        try {
            const res = await axios.post(`${BACKEND_URL}/document/share`, {
                docId: doc,
    ...
    const saveVersion = async () => {
        ...
        try {
            const res = await axios.post(`${BACKEND_URL}/document/version`, {
                docId: currDoc?.id,
    ...
    const saveDocument = async () => {
        ...
        setUnsaved(false);
        try {
            await axios.put(`${BACKEND_URL}/document/save`, {
                docId: currDoc?.id,
                title: currDoc?.title,
                content: text
            }, {
    ...
    const rollbackToVersion = async () => {
        ...
        try {
            const res = await axios.post(`${BACKEND_URL}/document/rollback/${doc}`, {
    ...
    const generateShareLink = async (role : string) => {
        ...
        try {
            const res = await axios.post(`${BACKEND_URL}/document/generate-link`, {
            ...
            .then(() => {
                toast({
                    ...
                    description: "Share link generated and copied to clipboard successfully",
                    duration: 2000
                });
    ...
    const deleteVersion = async (versionId: string) => {
        try {
            await axios.delete(`${BACKEND_URL}/document/delete-version/${versionId}`, {
                headers: {
                    Authorization: `Bearer ${localStorage.getItem('token')}
                `,
                },
    ...
    return (
        ...
        <Navbar />
         <div className="container max-w-[1500px] mx-auto py-20 min-h-screen">
                    ...
                    {myRole === 'VIEWER' ? <Badge className="bg-gray-50 mr-2"><p className="text-red-600 font-semibold flex justify-center items-center gap-2"><Lock size={20} /> READ ONLY</p></Badge> : null}
                    </div>
                ...
                <div className="mb-4 w-full md:w-8/12">
                    ...
                    <Card className="bg-white h-full shadow-md border-blue-200 overflow-hidden">
                            ...
                            <ReactQuill 
                                    theme="snow"
                                    value={text} 
                                    onChange={(value,delta,source : any) => sendData(value, delta,source)} 
                                    modules={modules}
                ...
                <Dialog open={shareDialogOpen} onOpenChange={setShareDialogOpen}>
                    <DialogContent className="sm:max-w-[425px] bg-white">
                        <DialogHeader>
                            <DialogTitle className="text-blue-800">Share Document</DialogTitle>
                        ...
                        <div className="grid gap-4 py-4">
                            <div className="grid grid-cols-4 items-center gap-4">
                                <Label htmlFor="role" className="text-blue-800">Access Level</Label>
                                <Select 
                ...
                <Dialog open={versionDialogOpen} onOpenChange={setVersionDialogOpen}>
                                        <DialogContent className="sm:max-w-[425px] bg-white">
                                            ...
                                            <DialogFooter className="flex flex-row items-center justify-center gap-2">
                                                ...
                                                >
                                                    {versionLoading ? <p className="flex items-center gap-2"><Loader2 className="animate-spin" /> Saving..</p> : <p className="flex items-center gap-1"><History className="mr-2 h-4 w-4" /> Save Version</p>}
                                                </Button>
                                            </DialogFooter>
                                        </DialogContent>
                                    </Dialog>
            ...
            <Dialog open={rollbackConfirmDialogOpen} onOpenChange={setRollbackConfirmDialogOpen}>
                <DialogContent className="sm:max-w-[425px] bg-white">
                    <DialogHeader>
                       ...
                       You are about to roll back the document to the version : <p className="font-semibold text-red-700">{rollbackVersionId}</p>This action will replace the current content of the document with the content from the chosen version. All unsaved changes in the current document will be lost.
                    ...
                    <DialogFooter>
                        ...
                        >
                            {rollbackLoading ? <p className="flex items-center gap-2"><Loader2 className="animate-spin" /> Rolling back..</p> : "Roll back"}
                        </Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>
